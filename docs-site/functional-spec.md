...
|                        | テクニカルチャート分析          | ローソク足、移動平均線、RSI、ボリンジャーバンドの表示と対話的分析（画面遷移）     | Web UI (画面)      |
|                        | 運用状況・投資日記              | 売買履歴の記録、投資判断メモの記述、パフォーマンス（損益）計算、CSV出力           | Web UI (画面)      |
|                        | 過去のAIレポート                | 配信された日次AI分析レポートのアーカイブ閲覧および検索                            | Web UI (画面)      |
| **ツール・計算機能**   | ポジションサイジング計算機      | 総資産・許容リスク率・損切幅に応じた最適購入株数・投入金額の算出                  | Web UI (モーダル)  |
|                        | 銘柄詳細チャートダイアログ      | 推奨・保有銘柄一覧から画面遷移せずにチャートをクイック確認するダイアログ           | Web UI (モーダル)  |
...
- **一覧表示項目**: 銘柄コード、銘柄名、セクター、トレード区分（短期/中期）、アクションシグナル、AIスコア、推奨価格、現在値、騰落率、目標/損切株価。
- **検索・フィルター**: 算出期間（全期間、直近1日、1週間〜1ヶ月等）による絞り込み、現在値一括更新機能。
- **アクション機能**:
  - `エントリー`: ポジションサイジング計算モーダルを開き購入株数を算出・保有登録。
  - `チャート`: 該当銘柄のテクニカルチャート分析（画面遷移またはモーダル）を表示。
  - `ニュース`: 該当銘柄に関連する最新適時開示・ニュース一覧モーダルを表示。
  - `行クリック`: 意思決定メモ（エントリー理由・コメント）表示モーダルを起動。
...
#### 3.2.4 テクニカルチャート分析画面（画面遷移）

サイドバーメニュー「📈 チャート分析」から画面遷移して利用する、主要分析ビューです。
...
- **表示機能**:
  ...
  - サブチャート：RSI (14日)、出来高
- **対話操作**:
  - 銘柄コード指定検索（任意の銘柄を検索・分析可能）
  - 期間切替（1ヶ月、3ヶ月、6ヶ月、1年）
  - 指標の表示/非表示トグル

> **補足**: 推奨銘柄やエントリー中銘柄の一覧テーブル上で各銘柄の「📈 チャート」ボタンをクリックした場合は、現在画面の上にオーバーレイで一時表示される**「銘柄詳細チャートモーダル (RecommendationChartModal)」**が起動します。画面遷移することなく、該当銘柄のテクニカルチャートをクイック参照できます。
