...
本書は、**【ワークフロー B】Web UI直接操作型 運用ワークフロー**の専用操作手順書です。  
Web UI画面を直接操作して、マクロ環境・買い環境スコア・市場アノマリーの分析から、適正購入株数の算出、トレーリングストップ・出口シグナル監視、取引履歴・投資日記の記録まで、一連のトレード運用を実行する手順を解説します。

> 💡 **ワークフローの使い分け**
> - **ワークフロー B (本書)**: Webダッシュボード上で分析・チャート閲覧・適正株数計算・結果確認・振り返りを直接操作して行いたい投資家向け。
> - **ワークフロー A**: 毎朝・日中に届くAIメール通知を起点に手早くトレードを行いたい方は **[メール受信・取引注文運用手順書 (MANUAL-EXT-002)](./02_email_order_manual.md)** を参照してください。
...
![意思決定ログモーダル](./images/modal_06_memo.png)
...
![意思決定ログ編集モーダル](./images/modal_06_memo.png)
...
![取引履歴編集モーダル](./images/modal_09_trade_edit.png)
